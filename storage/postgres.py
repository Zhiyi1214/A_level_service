...
class PostgresStore:
    ...
    def get(
        ...
        *,
        ...
        if message_limit is None:
            ...
            dname = conv.dify_conversation_name or ''
            total = len(messages)
            return {
                'id': conv.id,
    ...
    def get_summary(self, session_id: str) -> dict | None:
        ...
        return {
            ...
            'source_name': conv.source_name,
            'upstream_conversation_id': conv.upstream_conversation_id,
            'dify_conversation_name': conv.dify_conversation_name or '',
        }
    ...
    def upsert_user_from_provider(
        ...
        """INSERT 用户 + identity；若 (provider, provider_subject) 已存在则 ON CONFLICT 跳过 identity 并复用已有 user。"""
        now = datetime.now(timezone.utc)
        uid = str(uuid.uuid4())
        db.session.execute(
        ...
        stmt = (
            ...
            .on_conflict_do_nothing(constraint='uq_user_identity_provider_subject')
            .returning(UserIdentity.user_id)
        )
        row = db.session.execute(stmt).first()
        if row is None:
            db.session.execute(delete(User).where(User.id == uid))
            existing_uid = db.session.scalar(
            ...
            if existing_uid is None:
                raise RuntimeError(
                    'upsert_user_from_provider: conflict but identity row missing'
                )
            user = db.session.get(User, existing_uid)
            if user is not None:
                user.email = email
